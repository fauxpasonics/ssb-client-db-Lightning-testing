...
--base set
		SELECT 
			...
			, concat(c.name_first, ' ', c.name_last) as [FullName]
			...
			, 'TM-' + CAST(c.acct_id AS NVARCHAR(100)) + ':' + CAST(c.cust_name_id AS NVARCHAR(100)) AS[ExtAttribute6] 
		...
		--LEFT JOIN dbo.ASU_TktSystemConversion_DistinctPacIDs_20170703 m ON m.TM_SSID = CAST(c.acct_id AS NVARCHAR(25)) + ':' + CAST(c.cust_name_id AS NVARCHAR(25))
		
		WHERE 1=1

			AND (upd_date > DATEADD(DAY,-3,GETDATE()) OR add_date > DATEADD(DAY,-3,GETDATE()))

...
AS
    ...
    FROM    ( SELECT    ssbid.SSB_CRMSYSTEM_CONTACT_ID
		...
		--, DimSaleCode.SalesCodeClass
					  ...
					  , CASE WHEN DimEvent.EventDate = (SELECT MAX(EventDate) FROM ro.vw_DimEvent WHERE EventDate < CONVERT(DATE, GETDATE()))
					  THEN 1 ELSE 0 END AS Is_Prev_Game
					  , CASE WHEN DimEvent.EventDate = (SELECT MIN(EventDate) FROM ro.vw_DimEvent WHERE EventDate >= CONVERT(DATE,GETDATE()))
					  THEN 1 ELSE 0 END AS Is_Next_Game

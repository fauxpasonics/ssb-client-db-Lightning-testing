...
AS
    ( SELECT    ssbid.SSB_CRMSYSTEM_CONTACT_ID
              ...
              , DimScanGate.DimScanGateId AS Scan_Gate
         --     , fa.Channel
      FROM      dbo.FactAttendance_V2 fa
                ...
                INNER JOIN dbo.dimcustomer DimCustomer WITH ( NOLOCK ) ON DimCustomer.DimCustomerId = fa.DimTIcketCustomerId
                                                              AND DimCustomer.CustomerType = 'Primary'
                                                              AND DimCustomer.SourceSystem = 'TM'
                INNER JOIN dbo.dimcustomerssbid ssbid WITH ( NOLOCK ) ON ssbid.DimCustomerId = fa.DimTicketCustomerId
				...
				INNER JOIN ro.vw_DimScanGate DimScanGate WITH ( NOLOCK ) ON DimScanGate.DimScanGateId = fa.DimScanGateId

...
CREATE VIEW [etl].[vw_DynamicsContact_LoadDimCustomer] AS ( --Change "Source" to the correct source
 
    SELECT *
    /*Name*/
    , HASHBYTES('sha2_256',
...
--base set
        SELECT
            'Lightning' AS [SourceDB] --Client Source Database such as Raiders or Raiders_Reporting
...
 
            /*Standard Attributes*/
            , CAST(ContactID AS NVARCHAR(50)) [SSID] -- Required
...
            , CAST(z.owneridname AS NVARCHAR (50)) AS [AccountRep] 
            , CAST(company AS NVARCHAR (50)) AS [CompanyName] 
...
 
            /*AddressPrimary*/
            , address1_line1 AS [AddressPrimaryStreet] --CASE WHEN dc.DimCustomerId IS NULL THEN  NULLIF(ISNULL(address1_line1,'') + ' ' + ISNULL(address1_line2,''),' ') ELSE address1_line1 END AS [AddressPrimaryStreet] -- dont load suites combine addresses with multiple lines and load to street
...
 
            /*Email*/
            , NULLIF(CAST(emailaddress1 AS NVARCHAR(256)), 'Unknown') AS [EmailPrimary]
            , NULLIF(CAST(emailaddress2 AS NVARCHAR(256)), 'Unknown') AS [EmailOne]
            , NULLIF(CAST(emailaddress3 AS NVARCHAR(256)), 'Unknown') AS [EmailTwo]
 
            /*Extended Attributes*/
            , CAST(str_number AS NVARCHAR(100)) AS[ExtAttribute1] --nvarchar(100)
            , contactid AS[ExtAttribute2] --nvarchar(100)
            , NULL AS[ExtAttribute3] --nvarchar(100)
            , NULL AS[ExtAttribute4] --nvarchar(100)
            , NULL AS[ExtAttribute5] --nvarchar(100)
			, CASE WHEN z.new_SSBCRMSYSTEMSSIDWinner IS NULL OR z.new_ssbssidwinnersourcesystem IS NULL THEN NULL
			   WHEN z.new_ssbssidwinnersourcesystem = 'Eloqua' THEN 'ELO-' + z.new_SSBCRMSYSTEMSSIDWinner
			   WHEN z.new_ssbssidwinnersourcesystem = 'TM' THEN 'TM-' + z.new_ssbcrmsystemssidwinner
			   ELSE NULL END  AS [ExtAttribute6] --nvarchar(100)
...
            , z.createdby [SSCreatedBy] --createdby
...
            , CASE WHEN str_number LIKE '%|%' AND (SELECT value FROM [dbo].[fn_Split](str_number,'| ') WHERE position = 1) IS NOT NULL
				THEN  (SELECT value FROM [dbo].[fn_Split](str_number,'| ') WHERE position = 1) 
				WHEN str_number LIKE '%,%' AND (SELECT value FROM [dbo].[fn_Split](str_number,', ') WHERE position = 1) IS NOT NULL
				THEN  (SELECT value FROM [dbo].[fn_Split](str_number,', ') WHERE position = 1) 		
				WHEN ISNULL(str_number,'') = '' THEN NULL	
				ELSE RTRIM(str_number)
				END AS [AccountId]
            , 0 AS IsBusiness
			, z.statecode AS IsDeleted
			, z.str_number
			, CASE WHEN str_number LIKE '%|%' AND (SELECT value FROM [dbo].[fn_Split](str_number,'| ') WHERE position = 1) IS NOT NULL
			THEN RTRIM('TM-' + (SELECT value FROM [dbo].[fn_Split](str_number,'| ') WHERE position = 1))
			WHEN str_number LIKE '%,%' AND (SELECT value FROM [dbo].[fn_Split](str_number,', ') WHERE position = 1) IS NOT NULL
			THEN RTRIM('TM-' + (SELECT value FROM [dbo].[fn_Split](str_number,', ') WHERE position = 1))
				WHEN ISNULL(str_number,'') = '' THEN NULL	
				ELSE 'TM-' + RTRIM(str_number)
			END customer_matchkey 
...
        --SELECT COUNT(*)           
        FROM Lightning_reporting.prodcopy.contact z WITH (NOLOCK) 
		LEFT JOIN dimcustomer dc ON CAST(z.contactid AS NVARCHAR(100)) = dc.ssid 
        WHERE 1=1 --AND contactid = '92A8512D-8145-E211-81C3-005056AA001E'
			AND (DATEDIFF(DAY,z.modifiedon,GETDATE())<=3 OR DATEDIFF(DAY,z.copyloaddate,GETDATE())<=3) --This needs to be uncommented out after the first load, update the date fields
    ) a
 
)

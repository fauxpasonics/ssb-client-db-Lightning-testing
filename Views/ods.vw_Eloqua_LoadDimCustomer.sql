...
CREATE VIEW [ods].[vw_Eloqua_LoadDimCustomer] AS ( --Change "Source" to the correct source
...
--base set
        SELECT
            ...
            , CAST(z.Company AS NVARCHAR (50)) AS [CompanyName] 
            , NULL AS [SalutationName]
            , NULL AS [DonorMailName]
            , NULL AS [DonorFormalName]
            , CAST(NULL AS DATE) AS [Birthday] -- field is sensitive see writup in sharepoint for an example of how complicated it can get
            , NULL AS [Gender] 
            , 0 [MergedRecordFlag]
            , NULL [MergedIntoSSID]
            ...
            , NULL AS [Suffix]
--          , z.C_FirstAndLastName AS [Fullname]		--	DCH 2017-12-29
			, NULL AS [Fullname]						--	DCH 2017-12-29
            --, c.name_title as [Title]
 
            /*AddressPrimary*/
            , CASE 
				WHEN ISNULL(z.Address2, '') = ''  AND ISNULL(z.Address3, '') = '' AND ISNULL(z.Address1, '') <> '' THEN z.Address1 
				WHEN ISNULL(z.Address3, '') = '' AND  ISNULL(z.Address1, '') <> '' AND ISNULL(z.Address2, '') <> '' THEN z.Address1 + ' ' + z.Address2
				WHEN ISNULL(z.Address3, '') = '' AND  ISNULL(z.Address1, '') = '' AND ISNULL(z.Address2, '') <> '' THEN z.Address2
				WHEN ISNULL(z.Address3, '') <> '' AND  ISNULL(z.Address1, '') <> '' AND ISNULL(z.Address2, '') <> '' THEN z.Address1 + ' ' + z.Address2 + ' ' + z.Address3
			  END AS [AddressPrimaryStreet] -- dont load suites combine addresses with multiple lines and load to street
            , z.City AS [AddressPrimaryCity] 
            , z.C_State_Prov  AS [AddressPrimaryState] 
            , z.PostalCode AS [AddressPrimaryZip] 
            , NULL AS [AddressPrimaryCounty]
            , z.Country  AS [AddressPrimaryCountry] 
             
            , z.Address1 AS [AddressOneStreet]
            ...
            /*Phone*/
            , LEFT(COALESCE(z.MobilePhone, z.BusinessPhone),25) AS [PhonePrimary]
            , LEFT(NULL,25) AS [PhoneHome]
            , LEFT(z.MobilePhone,25) AS [PhoneCell]
            , LEFT(z.BusinessPhone,25) AS [PhoneBusiness]
            , LEFT(z.Fax,25) AS [PhoneFax]
            , LEFT(NULL,25) AS [PhoneOther]
 
            /*Email*/
            , CAST(z.EmailAddress AS NVARCHAR(256)) AS [EmailPrimary]
            ...
            , 'ELO-' + CAST(z.ID AS NVARCHAR(95)) AS[ExtAttribute6] --nvarchar(100)
            ...
            , z.CreatedAt [SSCreatedDate] --createddate
            , z.UpdatedAt [SSUpdatedDate] --lastmodifiedbyid
            , GETDATE () AS createddate 
            , GETDATE () AS updateddate
            , NULL [AccountId]
            , 0 AS IsBusiness
			, 0 AS IsDeleted
            , CASE WHEN ISNULL(NULLIF(CAST(z.C_Archtics_ID1 AS NVARCHAR(100)),'null'),'') != '' AND ISNULL(CAST(z.C_Archtics_ID1 AS NVARCHAR(100)),'') NOT LIKE '%|%' 
			THEN RTRIM(CONCAT('TM-',CAST(z.C_Archtics_ID1 AS NVARCHAR(100)))) ELSE 'ELO-' + CAST(z.ID AS NVARCHAR(100)) END AS [Customer_Matchkey] 
			-- Use when updating matchkeys for attribute 6--, CASE WHEN ISNULL(z.C_STH_Account_Number1,'') != '' THEN CONCAT('TM-',CAST(z.C_STH_Account_Number1 AS INT)) ELSE NULL END AS [Customer_Matchkey] 
        ...
        --SELECT COUNT(*)           
        FROM ods.Eloqua_Contact z WITH (NOLOCK) 
        WHERE 1=1
        AND DATEDIFF(DAY,z.UpdatedAt,GETDATE())<=10 OR DATEDIFF(DAY,z.CreatedAt,GETDATE())<=10 --This needs to be uncommented out after the first load, update the date fields

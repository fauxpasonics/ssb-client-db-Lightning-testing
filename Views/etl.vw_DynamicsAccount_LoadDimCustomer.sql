...
 
            /*AddressPrimary*/
            , address1_line1 AS [AddressPrimaryStreet] -- dont load suites combine addresses with multiple lines and load to street
...
            , CASE WHEN z.new_SSBCRMSYSTEMSSIDWinner IS NULL OR z.new_SSBCRMSystemSSIDWinnerSourceSystem IS NULL THEN NULL
			   WHEN z.new_SSBCRMSystemSSIDWinnerSourceSystem = 'Eloqua' THEN 'ELO-' + z.new_SSBCRMSYSTEMSSIDWinner
			   WHEN z.new_SSBCRMSystemSSIDWinnerSourceSystem = 'TM' THEN 'TM-' + SUBSTRING(new_SSBCRMSYSTEMSSIDWinner,1,CHARINDEX(':',new_SSBCRMSYSTEMSSIDWinner)-1)
			   WHEN z.new_SSBCRMSystemSSIDWinnerSourceSystem = 'TM-Suites' THEN 'TMS-' + z.new_SSBCRMSYSTEMSSIDWinner
			   ELSE NULL END AS[ExtAttribute6] --nvarchar(100)
            , NULL AS[ExtAttribute7] --nvarchar(100)
            , CASE WHEN z.accountnumber IS NOT NULL THEN 'TMS-' + CAST(z.accountnumber AS NVARCHAR(97)) ELSE NULL END AS[ExtAttribute8] --nvarchar(100)
...
        --SELECT COUNT(*)           
        FROM Lightning_Reporting.prodcopy.account z WITH (NOLOCK) 
        WHERE z.emailaddress1='Unknown'
        OR( DATEDIFF(DAY,z.modifiedon,GETDATE())<=3 OR DATEDIFF(DAY,z.createdon,GETDATE())<=3 OR DATEDIFF(DAY,z.copyloaddate,GETDATE())<=3) --This needs to be uncommented out after the first load, update the date fields
    ) a
 
)
 

...
--EXEC [api].[CRM_Attendance]


--SELECT ssbid.SSB_CRMSYSTEM_CONTACT_ID FROM dbo.FactAttendance_V2 f
--JOIN dbo.DimTicketCustomer_V2 t
--ON t.DimTicketCustomerId = f.DimTicketCustomerId
--JOIN dbo.DimCustomer d	
--ON d.AccountId = t.ETL__SSID_TM_acct_id AND d.SourceSystem = 'TM'
...
-- ==============================
        ...
        FROM    ( SELECT    dc.DimCustomerId AS AttendedDimCustomerID,
				...
				--SELECT COUNT(*)
				  FROM      dbo.FactInventory_V2 fi (NOLOCK)
                            ...
                            INNER JOIN dbo.DimEventHeader_V2 DimEventHeader WITH ( NOLOCK ) ON DimEventHeader.DimEventHeaderId = DimEvent.DimEventHeaderId
                            INNER JOIN dbo.DimSeasonHeader_V2 DimSeasonHeader  WITH ( NOLOCK ) ON DimSeasonHeader.DimSeasonHeaderId = DimEventHeader.DimSeasonHeaderId
							Left JOIN dbo.FactAttendance_V2 FactAttendance WITH (NOLOCK) ON factattendance.FactAttendanceId = fi.FactAttendanceId
							left JOIN dbo.DimScanGate_V2 scangate WITH (NOLOCK) ON scangate.dimscangateID = factattendance.dimscangateID
		...
		WHERE   ssbid.SSB_CRMSYSTEM_CONTACT_ID 
		--= '939E2C4A-FD6D-4410-BBA6-DED038176140'

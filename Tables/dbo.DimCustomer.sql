...
GO
	...
	CREATE TRIGGER [dbo].[FuzzyName_AddressCity]
	ON [dbo].[DimCustomer]
	AFTER UPDATE
	AS 
	BEGIN
	IF UPDATE(FuzzyNameGUID) OR UPDATE (AddressPrimaryStreet) OR UPDATE (AddressPrimaryCity)
		BEGIN
...
GO
	...
	CREATE TRIGGER [dbo].[FuzzyName_AddressZip]
	ON [dbo].[DimCustomer]
	AFTER UPDATE
	AS 
	BEGIN
	IF UPDATE(FuzzyNameGUID) OR UPDATE (AddressPrimaryStreet) OR UPDATE (AddressPrimaryZip)
		BEGIN
		UPDATE dbo.DimCustomer
		SET matchkey_updatedate = CURRENT_TIMESTAMP
		FROM dbo.DimCustomer a
...
GO
CREATE NONCLUSTERED INDEX [IX_DimCustomer_NameIsCleanStatus] ON [dbo].[DimCustomer] ([NameIsCleanStatus], [IsDeleted]) INCLUDE ([DimCustomerId])
GO
